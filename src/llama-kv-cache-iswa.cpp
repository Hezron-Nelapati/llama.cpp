...
llama_kv_cache_iswa::llama_kv_cache_iswa(
    ...
    llama_kv_cache_iswa(model, model.hparams, type_k, type_v, v_trans, offload, swa_full, unified,
            kv_size, n_seq_max, n_ubatch, n_pad, mem_other, filter, reuse, share, kv_size_init, grow_margin_mib) {
}
...
llama_kv_cache_iswa::llama_kv_cache_iswa(
    ...
    const  layer_share_cb & share,
                 uint32_t   kv_size_init,
                 uint32_t   grow_margin_mib) : unified(unified) {
    ...
    kv_base = std::make_unique<llama_kv_cache>(
            ...
            0, LLAMA_SWA_TYPE_NONE, mem_other_base, filter_base, reuse, share, "", kv_size_init, grow_margin_mib);

    LLAMA_LOG_INFO("%s: creating     SWA KV cache, size = %u cells\n", __func__, size_swa);

    // a full-size SWA cache holds as much as the base cache, so it grows the same way
    kv_swa = std::make_unique<llama_kv_cache>(
            model, hparams, type_k, type_v,
            v_trans, offload, unified, size_swa, n_seq_max, n_pad,
            hparams.n_swa, hparams.swa_type, mem_other_swa, filter_swa, reuse, share, "", swa_full ? kv_size_init : 0, grow_margin_mib);
}
...
bool llama_kv_cache_iswa::get_can_shift() const {
    return kv_base->get_can_shift() &&
           ...
           kv_base->get_size_max() == kv_swa->get_size_max();
}

uint32_t llama_kv_cache_iswa::get_layout_gen() const {
    return kv_base->get_layout_gen() + kv_swa->get_layout_gen();
}

bool llama_kv_cache_iswa::shrink() {
    const bool base = kv_base->shrink();
    const bool swa  = kv_swa->shrink();
