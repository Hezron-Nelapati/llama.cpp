...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        // only the scale can be invalid: a code is a bit pattern and every codebook entry is
        // a finite constant, so the product is finite iff the scale is. The blocks differ in
        // width, so the cases cannot share a body.
#define NEURON_V_VALIDATE(SFX)                                                             \
        case GGML_TYPE_NEURON_V##SFX:                                                      \
            {                                                                              \
                const block_neuron_v##SFX * q = (const block_neuron_v##SFX *) data;        \
                for (size_t i = 0; i < nb; ++i) {                                          \
...
// Generic bit field access. Widths run to 2m+3 = 19 bits, so a field can span four bytes
// once its offset is counted. Each byte beyond the first is read ONLY when the field
// actually reaches into it -- the offsets are compile-time constant at every call site
// once the loops unroll, and an unconditional read would run past the final group of the
// final block.
static inline uint32_t neuron_bits(const uint8_t * GGML_RESTRICT qs, int bit, int nbits) {
...
static inline void neuron_putbits(uint8_t * GGML_RESTRICT qs, int bit, int nbits, uint32_t c) {
    for (int i = 0; i < nbits; ++i) {
        const int b = bit + i;
        if (c & (1u << i)) qs[b >> 3] |=  (uint8_t)(1u << (b & 7));
        else               qs[b >> 3] &= (uint8_t)~(1u << (b & 7));
    }
}

// Alternating refit. Round-to-nearest picks the pair of best codes; the best PAIR of codes
// is a different thing, because a slightly worse angle can admit a much better magnitude.
//
...
//
// Objective is plain Frobenius. Weighting by r^-2 is scale-free but unbounded as r -> 0,
// and was measured at 27.06 -> 30.19 perplexity at 4 bits: the relative behaviour the
// model needs comes from coding magnitude in LOG space, not from the norm the levels are
// fitted in. The Python M2 step (Newton on the shared level shape) is not ported -- it
// moves a per-tensor `warp` table this format does not carry.
//
// ENCODER-ONLY: lo, mid and hi are written either way, so the format, both decoders and
// every kernel are untouched. Its value scales with how much damage there is to undo --
// measured at -1.7% perplexity at m3 and 0.0% at m5, where round-to-nearest is already
// finding near-optimal codes.
//
// Angle candidates are loop-invariant (theta never moves), so they and log(r cos d) are
// computed once up front and the iteration holds no trig at all.
#define NEURON_FT_ITERS 16
#define NEURON_MAXP (QK_NEURON / 2)

static void neuron_refit_block(const float * GGML_RESTRICT rr,
                               const float * GGML_RESTRICT th,
                               int np, int nlv, int A,
                               float * GGML_RESTRICT plo,
                               float * GGML_RESTRICT pmid,
                               float * GGML_RESTRICT phi,
                               int * GGML_RESTRICT out_ac,
                               int * GGML_RESTRICT out_mc) {
    const int last = nlv - 1;
    int   cand[NEURON_MAXP][2];
    float ccd [NEURON_MAXP][2];
    float clt [NEURON_MAXP][2];
    for (int p = 0; p < np; ++p) {
        const float ci = (th[p] + (float)M_PI) / (2.0f * (float)M_PI) * A;
        const int   f0 = (int)floorf(ci);
        for (int c = 0; c < 2; ++c) {
            const int a = (((f0 + c) % A) + A) % A;      /* A is even: mod keeps parity */
            const float cd =
                cosf(th[p] - ((float)a / (float)A * 2.0f*(float)M_PI - (float)M_PI));
            cand[p][c] = a;
            ccd [p][c] = cd;
            clt [p][c] = logf(fmaxf(rr[p] * cd, 1e-30f));
        }
    }
    float lo  = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(*plo));
    float mid = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(*pmid));
    float hi  = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(*phi));
    for (int it = 0; it < NEURON_FT_ITERS; ++it) {
        float lvf[1 << 8];
        for (int j = 0; j < nlv; ++j) {
            lvf[j] = expf(neuron_ladder((float)j / (float)last, lo, mid, hi));
        }
        float num = 0.0f, den = 0.0f;
        for (int s = 0; s < np / 2; ++s) {            /* one joint angle code per 2 pairs */
            const int pa = 2*s, pb = pa + 1;
            float bestsum = INFINITY;
            int   ba = 0, bb = 0, bca = 0, bcb = 0;
            float bAa = 0.0f, bAb = 0.0f, bra = 0.0f, brb = 0.0f;
            for (int ca = 0; ca < 2; ++ca) {
                for (int cb = 0; cb < 2; ++cb) {
                    /* the checkerboard admits only the even-sum half of the grid */
                    if ((cand[pa][ca] + cand[pb][cb]) & 1) continue;
                    float ea = INFINITY, eb = INFINITY;
                    int   ja = 0, jb = 0;
                    float ra = 0.0f, rb = 0.0f;
                    for (int w = 0; w < 2; ++w) {
                        const int   p  = w ? pb : pa;
                        const int   cx = w ? cb : ca;
                        const float rc = rr[p], cd = ccd[p][cx];
                        const float u  = neuron_coord(clt[p][cx], lo, mid, hi);
                        int c0 = (int)lroundf(u * (float)last);
                        if (c0 < 0) c0 = 0; if (c0 > last) c0 = last;
                        for (int dl = -1; dl <= 1; ++dl) {
                            int cc = c0 + dl;
                            if (cc < 0) cc = 0; if (cc > last) cc = last;
                            const float rq  = lvf[cc];
                            const float err = rc*rc - 2.0f*rc*rq*cd + rq*rq;
                            if (w == 0) { if (err < ea) { ea = err; ja = cc; ra = rq; } }
                            else        { if (err < eb) { eb = err; jb = cc; rb = rq; } }
                        }
                    }
                    if (ea + eb < bestsum) {
                        bestsum = ea + eb;
                        ba = cand[pa][ca]; bb = cand[pb][cb];
                        bca = ja; bcb = jb;
                        bAa = rr[pa]*ccd[pa][ca]; bAb = rr[pb]*ccd[pb][cb];
                        bra = ra; brb = rb;
                    }
                }
            }
            out_ac[pa] = ba;  out_ac[pb] = bb;
            out_mc[pa] = bca; out_mc[pb] = bcb;
            num += bAa*bra + bAb*brb;
            den += bra*bra + brb*brb;
        }
        float sc = den > 0.0f ? num / fmaxf(den, 1e-30f) : 1.0f;
        if (sc < 0.5f) sc = 0.5f; if (sc > 2.0f) sc = 2.0f;
        const float shift = logf(sc);
        lo  = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(lo  + shift));
        mid = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(mid + shift));
        hi  = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(hi  + shift));
        if (mid <= lo) mid = lo + 1e-4f;
        if (mid >= hi) mid = hi - 1e-4f;
    }
    *plo = lo; *pmid = mid; *phi = hi;
}

static void neuron_encode_block(const float * GGML_RESTRICT xb, int LP,
                                ggml_half * GGML_RESTRICT plo,
                                ggml_half * GGML_RESTRICT pmid,
                                ggml_half * GGML_RESTRICT phi,
    ...
    float lr[NEURON_MAXP], th[NEURON_MAXP], rr[NEURON_MAXP];
    ...
    if (mid <= lo) mid = lo + 1e-4f;
    if (mid >= hi) mid = hi - 1e-4f;
    int acs[NEURON_MAXP], mcs[NEURON_MAXP];
    neuron_refit_block(rr, th, np, nlv, A, &lo, &mid, &hi, acs, mcs);
    *plo = GGML_FP32_TO_FP16(lo);
    *pmid = GGML_FP32_TO_FP16(mid);
    *phi = GGML_FP32_TO_FP16(hi);
    memset(qs, 0, (size_t)(QK_NEURON / 32) * GBY);
    for (int g = 0; g < np / NEURON_GRP; ++g) {
        uint8_t * grp = qs + (size_t)g * GBY;
        uint8_t * ang = grp + AOFF;
        const int p0 = g * NEURON_GRP;
        for (int t = 0; t < NEURON_GRP; ++t) {
            neuron_putbits(grp, t * LP, LP, (uint32_t)mcs[p0 + t]);
        }
        for (int t = 0; t < NEURON_GRP / 2; ++t) {
            const int ia = acs[p0 + 2*t], ib = acs[p0 + 2*t + 1];
            neuron_putbits(ang, t * AW, AW, ((uint32_t)ia << JSH) | ((uint32_t)ib >> 1));
        }
    }
}

static void neuron_decode_block(const uint8_t * GGML_RESTRICT qs, int LP,
                                float lo, float mid, float hi,
                                float * GGML_RESTRICT yb) {
    const int np  = QK_NEURON / 2;
    const int nlv = 1 << LP, last = nlv - 1;
    const int A   = NEURON_ANG(LP), AW = NEURON_AW(LP);
    const int GBY = NEURON_GBY(LP), AOFF = NEURON_AOFF(LP), JSH = NEURON_JSH(LP);
    const uint32_t JM = (1u << JSH) - 1u;
    for (int g = 0; g < np / NEURON_GRP; ++g) {
        const uint8_t * grp = qs + (size_t)g * GBY;
        const uint8_t * ang = grp + AOFF;
        const int p0 = g * NEURON_GRP;
        for (int t = 0; t < NEURON_GRP; ++t) {
            const int p  = p0 + t;
            const int mc = (int)neuron_bits(grp, t * LP, LP);
            const uint32_t v = neuron_bits(ang, (t >> 1) * AW, AW);
            const int ia = (int)(v >> JSH);
            const int ac = (t & 1) ? (int)(2u*(v & JM)) + (ia & 1) : ia;
            const float u  = (float)mc / (float)last;
            const float r  = expf(neuron_ladder(u, lo, mid, hi));
            const float t2 = (float)ac / (float)A * (2.0f*(float)M_PI) - (float)M_PI;
            yb[2*p]     = r * cosf(t2);
            yb[2*p + 1] = r * sinf(t2);
        }
    }
...
void quantize_row_neuron_m##LP##_ref(const float * GGML_RESTRICT x,                          \
    ...
    for (int i = 0; i < nb; ++i) {                                                           \
        neuron_encode_block(x + (size_t)i * QK_NEURON, (LP),                                 \
                            &y[i].lo, &y[i].mid, &y[i].hi, y[i].qs);                         \
    }                                                                                        \
}                                                                                            \
                                                                                             \
void dequantize_row_neuron_m##LP(const block_neuron_m##LP * GGML_RESTRICT x,                 \
    ...
    assert(k % QK_NEURON == 0);                                                              \
    for (int i = 0; i < nb; ++i) {                                                           \
        neuron_decode_block(x[i].qs, (LP), GGML_FP16_TO_FP32(x[i].lo),                       \
                            GGML_FP16_TO_FP32(x[i].mid), GGML_FP16_TO_FP32(x[i].hi),         \
                            y + (size_t)i * QK_NEURON);                                      \
    }                                                                                        \
}
...
// =======================================================================================
// neuron_v* -- d=2 vector quantisation, one code per pair into a fixed codebook.
//
// Encoding is nearest-neighbour over K codewords, then a per-block least-squares scale,
// alternated. The scale is what the polar family's M1 step did: round-to-nearest under a
// fixed scale is not the same as the best (scale, codes) pair, because a block's
// reconstruction is biased by which codes were picked.
//
//   argmin_c ||x - d*C[c]||^2  ==  argmax_c  2*d*<x,C[c]> - d^2*||C[c]||^2
//   d*       = sum <x, C[c]> / sum ||C[c]||^2
//
// The codebook is fitted on ABSMAX-normalised pairs and its coverage stops at radius ~1.36,
// while an absmax-normalised pair reaches sqrt(2). A least-squares scale is SMALLER than
// absmax, which pushes pairs further out still, so d is floored at the absmax value: the
// refit may only shrink the reconstruction, never the scale below what the codebook was
// trained to cover.
//
...
//
// which is a linear functional over the K points (cn[c], dot[c]), so the maximiser is always
// a vertex of their upper-left convex hull -- never an interior point. cn = ||C||^2 is a
// global constant, so the points can be pre-sorted by x ONCE for the whole model and the
// per-pair hull becomes a linear monotone chain rather than a sort. And the candidate scales
// are a sorted one-parameter family, so as g falls the optimum walks monotonically rightward
// along the hull: two pointers, no binary search.
//
//     per pair:   K dots + O(K) hull + O(H + NM) walk   ~ 2K
//     instead of: NM * K                                = 16K at v4/v5, 64K at v6
//
// Exact: the hull vertex provably attains the maximum. Ties may resolve to a different
// codeword of equal score than the exhaustive scan picked, so output is quality-equivalent
// rather than bit-identical -- same situation as the codeword-outermost reorder.
static void neuron_vq_scales(const float * GGML_RESTRICT cb, const float * GGML_RESTRICT cn,
                             const int * GGML_RESTRICT ord, int K,
                             float ax, float ay,
                             const float * GGML_RESTRICT gv, int NM,
                             int * GGML_RESTRICT out_c, float * GGML_RESTRICT out_sse,
                             float * GGML_RESTRICT dot, int * GGML_RESTRICT hull) {
    for (int c = 0; c < K; ++c) {
        dot[c] = ax * cb[2*c] + ay * cb[2*c + 1];
    }
    // upper-left hull over cn ascending: keep the Pareto staircase (y strictly rising), then
    // the concave chain on top of it
    int   nh   = 0;
    float ymax = -INFINITY;
    for (int i = 0; i < K; ++i) {
        const int c = ord[i];
        if (dot[c] <= ymax) {
            continue;                       // dominated: smaller cn already reaches this dot
        }
        ymax = dot[c];
        while (nh >= 2) {
            const int a = hull[nh-2], b = hull[nh-1];
            const float dx1 = cn[b] - cn[a], dy1 = dot[b] - dot[a];
            const float dx2 = cn[c] - cn[b], dy2 = dot[c] - dot[b];
            if (dy2 * dx1 < dy1 * dx2) {
                break;                      // still concave
            }
            --nh;                           // b is not a vertex
        }
        hull[nh++] = c;
    }
    // gv is descending, so the slope g/2 falls and the optimum only ever moves right
    int v = 0;
    const float xn = ax*ax + ay*ay;
    for (int j = 0; j < NM; ++j) {
        const float g = gv[j], sl = 0.5f * g;
        while (v + 1 < nh &&
               (dot[hull[v+1]] - dot[hull[v]]) >= sl * (cn[hull[v+1]] - cn[hull[v]])) {
            ++v;
        }
        const int c = hull[v];
        out_c[j]   = c;
        out_sse[j] = xn - (2.0f*g*dot[c] - g*g*cn[c]);
    }
}
...
static void neuron_vq##SFX##_init(void) {                                                  \
    ...
    /* the hull query walks codewords in cn order, and cn is a global constant, so         \
       this sort happens once per type rather than once per pair */                        \
    for (int c = 0; c < NEURON_VQ##SFX##_K; ++c) { g_vq##SFX##_ord[c] = c; }               \
    for (int a = 1; a < NEURON_VQ##SFX##_K; ++a) {          /* insertion sort: K is small */\
        const int  key = g_vq##SFX##_ord[a];                                               \
        const float kv = g_vq##SFX##_cn[key];                                              \
        int b = a - 1;                                                                     \
        while (b >= 0 && g_vq##SFX##_cn[g_vq##SFX##_ord[b]] > kv) {                        \
            g_vq##SFX##_ord[b+1] = g_vq##SFX##_ord[b];                                     \
            --b;                                                                           \
        }                                                                                  \
...
void quantize_row_neuron_v##SFX##_ref(const float * GGML_RESTRICT x,                       \
    ...
    for (int i = 0; i < nb; ++i) {                                                         \
        ...
        memset(y[i].sb, 0, NEURON_VQ_SBBY(NEURON_VQ##SFX##_SBB));                          \
        memset(y[i].qs, 0, NBY);                                                           \
        if (amax == 0.0f) {                                                                \
...
/* inner: exhaustive over the 16 representable multipliers, per sub-block.                 \
               Loop order is codeword-outermost so the codebook is streamed ONCE rather than\
               once per multiplier. The score 2g<x,C> - g^2||C||^2 shares <x,C> across every\
               multiplier, so each dot is computed once per (codeword, pair) and the 16    \
               multipliers then only scale register-resident values. Exact -- same codes, same\
               tie-breaking on first-max -- and 128x fewer codebook loads, which is what   \
               actually costs at K=4096 where the table is 32 KB and misses L1. */         \
            for (int sblk = 0; sblk < NEURON_VQ_NSB; ++sblk) {                             \
                const int NM = 1 << NEURON_VQ##SFX##_SBB;  /* 16 at v4/v5, 64 at v6 */     \
                float gv[64];                                                              \
                for (int j = 0; j < NM; ++j) {                                             \
                    gv[j] = d * NEURON_VQ##SFX##_SBT[j];   /* descending: the table is sorted */\
                }                                                                          \
                float tot[64];                                                             \
                int   cc[64][NEURON_VQ_SBV / 2];                                           \
                for (int j = 0; j < NM; ++j) { tot[j] = 0.0f; }                            \
                for (int t = 0; t < PSB; ++t) {                                            \
                ...
                float be = INFINITY;                                                       \
                for (int j = 0; j < NM; ++j) {                                             \
                    if (tot[j] < be) { be = tot[j]; bj = j; }                              \
                }                                                                          \
                for (int t = 0; t < PSB; ++t) { cs[sblk*PSB + t] = cc[bj][t]; }            \
                mi[sblk] = bj;                                                             \
            }                                                                              \
            ...
            /* fp16 is what gets stored, so converge on the value the decoder will see;    \
               and never below absmax, which is the radius the codebook covers */          \
            const float dn = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(num / den));              \
            if (!(dn > amax)) {                                                            \
                break;                                                                     \
            }                                                                              \
            d = dn;                                                                        \
        ...
        for (int sblk = 0; sblk < NEURON_VQ_NSB; ++sblk) {                                 \
            ...
            y[i].sb[bs_ >> 3] |= (uint8_t)(mi[sblk] << (bs_ & 7));                         \
            if (((bs_ & 7) + NEURON_VQ##SFX##_SBB) > 8) {                                  \
                y[i].sb[(bs_ >> 3) + 1] |= (uint8_t)(mi[sblk] >> (8 - (bs_ & 7)));         \
            }                                                                              \
        }                                                                                  \
        for (int p = 0; p < NP; ++p) {                                                     \
...
void dequantize_row_neuron_v##SFX(const block_neuron_v##SFX * GGML_RESTRICT x,             \
    ...
    for (int i = 0; i < nb; ++i) {                                                         \
        ...
        for (int p = 0; p < QK_NEURON / 2; ++p) {                                          \
            ...
            const int bs_ = sblk * NEURON_VQ##SFX##_SBB;                                   \
            uint32_t  sv  = (uint32_t) x[i].sb[bs_ >> 3];                                  \
            if (((bs_ & 7) + NEURON_VQ##SFX##_SBB) > 8) {                                  \
                sv |= (uint32_t) x[i].sb[(bs_ >> 3) + 1] << 8;                             \
            }                                                                              \
            ...
            yb[2*p + 1] = g * g_vq##SFX##_tbl[2*c + 1];                                     \
                                                                                           ...
                                                                                           \
size_t quantize_neuron_v##SFX(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst,   \
                              int64_t nrow, int64_t n_per_row,                             \
                              const float * quant_weights) {                               \
    GGML_UNUSED(quant_weights);                                                            \
    const size_t row_size = ggml_row_size(GGML_TYPE_NEURON_V##SFX, n_per_row);             \
    char * qrow = (char *) dst;                                                            \
    for (int64_t r = 0; r < nrow; ++r) {                                                   \
        quantize_row_neuron_v##SFX##_ref(src, (block_neuron_v##SFX *) qrow, n_per_row);    \
        src  += n_per_row;                                                                 \
        qrow += row_size;                                                                  \
    }                                                                                      \
    return nrow * row_size;                                                                \
...
// ---------------------------------------------------------------------------------------
// neuron_l4 -- lattice. One 4-bit level index per value, v4's block and scale search.
//
// Levels are symmetric, so a value is coded as sign + magnitude index, and the nearest
// magnitude for t = |x|/g is the count of boundaries below t. The reconstruction is the same
// count written as steps: mag[0] + sum_k (t > b[k]) * (mag[k+1] - mag[k]). No table read in
...
void quantize_row_neuron_l4_ref(const float * GGML_RESTRICT x, block_neuron_l4 * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t i = 0; i < nb; ++i) {
        ...
        for (int it = 0; it <= NEURON_L4_REFIT; ++it) {
            ...
            // no floor at amax: levels have no coverage radius, so clipping the largest value is
            // allowed when least squares prefers it
            const float dn = GGML_FP16_TO_FP32(GGML_FP32_TO_FP16(num / den));
            if (!(dn > 0.0f) || dn == d) {
                break;
            }
